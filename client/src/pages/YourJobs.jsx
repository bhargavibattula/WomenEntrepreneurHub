...
import { AiOutlineDelete, AiOutlinePlus, AiOutlineEdit, AiOutlineClose, AiOutlineBriefcase } from 'react-icons/ai'
            ...
            <div className='max-w-7xl mx-auto'>
                <div className='flex gap-5 flex-col'>
                    <div className="flex justify-between items-center mb-6">
                        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight uppercase">Your Job Posts</h1>
                        <Link to={"/job/create"} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl flex gap-2 items-center font-bold shadow-lg shadow-blue-600/20 transition-all active:scale-95">
                    ...
                    <div className='flex w-full gap-4 items-center mb-8'>
                        <input placeholder="Search your job posts..." type="text" className="w-full flex-1 p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all" />
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {jobs?.length > 0 ? (
                            jobs.map((job, index) => (
                                <div key={index} className="group w-full p-6 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md hover:border-blue-200 transition-all flex items-center justify-between">
                                    <div className='flex flex-col gap-2'>
                                        <span className='uppercase font-bold text-2xl text-blue-600 tracking-tight'>{job?.title}</span>
                                        <span className='text-slate-500 text-sm max-w-2xl line-clamp-2'>{job?.description}</span>
                                        <div className="flex gap-2 mt-2">
                                            <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-bold uppercase tracking-wider">{job?.employmentType}</span>
                                            <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-full text-xs font-bold uppercase tracking-wider">{job?.location}</span>
                                        </div>
                                    </div>
                            ...
                            <div className="text-center py-20 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                                <p className="text-slate-400 font-medium text-lg">No job posts found. Start hiring!</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="text-center mt-4">
                                ...
                                <p className="text-slate-500 mb-8">Are you sure you want to delete this job post? This action cannot be undone.</p>
